...
**EN — Live security checks (website/link targets / `app/utils/live_checks.py` + `app/utils/owasp_live.py`):** beyond source analysis, the website & domain engines run response-passive checks over the crawled pages — verbose/unhandled error disclosure (`VERBOSE-ERROR`, `UNHANDLED-ERROR`), cookie flag weaknesses (`COOKIE-NO-HTTPONLY/SECURE/SAMESITE`), insecure transport + missing HSTS (`INSECURE-TRANSPORT`, `HSTS-MISSING`), dangerous methods (`TRACE-ENABLED`), tech disclosure (`INFO-X-POWERED-BY`), OWASP attack-surface classification (`INFO-QUERY-SECRET`, `CSV-DOWNLOAD`, `UPLOAD-FORM`, `DESERIALIZE-ENDPOINT`, `XML-ENDPOINT`), plus server-level fingerprints `NIKTO-HEARTBLEED` (OpenSSL 1.0.1), live TLS-cert expiry, platform exposure and the Follina signature. A dedicated **OWASP community-vulnerabilities module** (`app/utils/owasp_live.py`) adds deterministic passive checks mapped to the OWASP list: login form sent via GET with password (`OWASP-LOGIN-GET`), password autofill (`OWASP-PW-AUTOFILL`), HTTPS mixed content (`OWASP-MIXED-CONTENT`), third-party script/style without SRI (`OWASP-EXTERNAL-NOSRI`), serialized-object magic bytes in client-observable data (`OWASP-DESER-MAGIC` — CWE-502), and low-entropy session IDs (`OWASP-SESSION-ENTROPY` — CWE-331/330/335).

**EN — Active injection probing (website/link):** the engine also sends *benign, read-only* injection markers (`${7*7777777}`, `{{7*7777777}}`, `<%= 7*7777777 %>`) to discovered query parameters and reports `INJ-LIVE-SSTI` only when the expression is evaluated (the unique marker `54444439` leaks back, and the raw marker is absent) and `INJ-LIVE-CRLF` on a reflected newline — covering Expression-Language / template injection and CRLF/response-splitting on live targets.

**ID — Cek keamanan live (target website/link / `app/utils/live_checks.py` + `app/utils/owasp_live.py`):** selain analisis source, engine website & domain menjalankan cek response-passive pada halaman yang di-crawl — disclosure error verbose/unhandled (`VERBOSE-ERROR`, `UNHANDLED-ERROR`), kelemahan flag cookie (`COOKIE-NO-HTTPONLY/SECURE/SAMESITE`), transport tidak aman + HSTS hilang (`INSECURE-TRANSPORT`, `HSTS-MISSING`), metode berbahaya (`TRACE-ENABLED`), disclosure teknologi (`INFO-X-POWERED-BY`), klasifikasi attack-surface OWASP (`INFO-QUERY-SECRET`, `CSV-DOWNLOAD`, `UPLOAD-FORM`, `DESERIALIZE-ENDPOINT`, `XML-ENDPOINT`), plus fingerprint server-level `NIKTO-HEARTBLEED` (OpenSSL 1.0.1), kedaluwarsa sertifikat TLS live, platform-exposure dan signature Follina. Modul khusus **kerentanan komunitas OWASP** (`app/utils/owasp_live.py`) menambahkan cek pasif deterministik yang dipetakan ke daftar OWASP: form login via GET dengan field password (`OWASP-LOGIN-GET`), autofill password (`OWASP-PW-AUTOFILL`), mixed content HTTPS (`OWASP-MIXED-CONTENT`), script/style pihak ketiga tanpa SRI (`OWASP-EXTERNAL-NOSRI`), magic-byte object serialized pada data yang terpapar klien (`OWASP-DESER-MAGIC` — CWE-502), dan session-id berentropi rendah (`OWASP-SESSION-ENTROPY` — CWE-331/330/335).

**ID — Probing injeksi aktif (website/link):** engine juga mengirim marker injeksi *benign & read-only* (`${7*7777777}`, `{{7*7777777}}`, `<%= 7*7777777 %>`) ke parameter query yang ditemukan dan melaporkan `INJ-LIVE-SSTI` hanya bila expression dievaluasi (marker unik `54444439` bocor & marker mentah absen) serta `INJ-LIVE-CRLF` bila newline terefleksi — mencakup Expression-Language/template injection dan CRLF/response-splitting pada target live.
...
| GET | `/tools` | Pentest tools catalog (Kali-style + OSINT, 332 tools, 35 categories, each with `features`) / Katalog tools pentest (Kali-style + OSINT, 332 tool, 35 kategori, masing-masing dengan fitur) |
...
```bash
cyense tools list                            # all 332 tools grouped by category
...
│       └── pentest-tools-catalog.md   ← pentest tools catalog + per-tool features PRD
├── dev/
...
- ✅ Pentest tools catalog (CLI + Website + API `/tools`): 332 tools · 35 kategori (Kali-style + OSINT), tiap tool dengan daftar fitur/kapabilitas, contoh penggunaan (`usage`), bookmark/referensi, dan tool terkait (`related`); pencarian `--feature`/search bar, filter kategori, detail per tool (`tools info`/`tools usage` + panel detail Website) / Katalog tools pentest (CLI + Website + API `/tools`): 332 tool · 35 kategori, tiap tool dengan fitur, contoh penggunaan, bookmark, dan tool terkait; pencarian fitur, filter kategori, detail per tool
