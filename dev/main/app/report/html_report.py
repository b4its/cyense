...
def _summary_chips(summary: dict[str, Any]) -> str:
    ...
    if "rejected_false_positives" in summary:
        extra.append(
            f"<div class='chip'>rejected FP <strong>{summary['rejected_false_positives']}</strong></div>"
        )
    if "files_scanned" in summary:
        extra.append(
            f"<div class='chip'>files scanned <strong>{summary['files_scanned']}</strong></div>"
        )
...
def _curl_of(request: dict[str, Any]) -> str:
    ...
    for key, value in (request.get("cookies") or {}).items():
...
details.finding summary{cursor:pointer;padding:12px 16px;display:flex;gap:10px;align-items:center;flex-wrap:wrap}
...
<footer>generated by Cyense 2.0.0 · string-builder HTML (no template engine) · credentials redacted</footer>
</body>
</html>"""
