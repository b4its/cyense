...
class GithubClient:
    ...
    async def get_repo_metadata(
        ...
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            resp = await client.get(url, headers={"Authorization": f"Bearer {self.redact_token(self._token)}"})
    ...
    async def download_tarball(self, owner: str, repo: str, ref: str) -> tuple[bytes, dict[str, Any]]:
